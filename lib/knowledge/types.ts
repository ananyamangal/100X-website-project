...
export interface KnowledgeArticle {
  ...
  order: number
  /**
   * Absolute canonical URL when the page is a derived view of content that lives
   * elsewhere (synced entries point at their source page). Absent = self-canonical.
   */
  canonicalUrl?: string
  /** Present only on articles generated by the Knowledge Base sync. */
  sync?: KnowledgeSyncMeta
}

export type KnowledgeSyncSource = "blogs" | "case_studies" | "past_performance" | "products"

export interface KnowledgeSyncMeta {
  source: KnowledgeSyncSource
  /** Source document id, or "all" for aggregate pages built from a whole collection. */
  sourceId: string
  /** Absolute URL of the page this entry was derived from. */
  sourceUrl: string
  /** Hash of the generated content (dates excluded): unchanged source = no rewrite. */
  hash: string
  syncedAt: string
  /**
   * "auto": published by the sync. "draft-review": held as a draft because the
   * content touches chemicals, dosing or safety; the owner reviews it first.
   */
  policy: "auto" | "draft-review"
  /** Why an entry was held for review (matched sensitive terms). */
  reasons: string[]
  /** Set when an admin edits a synced article; later syncs then leave it alone. */
  locked?: boolean
}
