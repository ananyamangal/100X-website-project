...
export const PROD_FILTER = {
  _test: { $ne: true },
  name:  { $not: { $regex: '^(playwright|precommit-\\d)|^test\\s|\\btest lead\\b|playwright\\s+(test|mobile)', $options: 'i' } },
  phone: { $nin: ['9000000001', '9000000002'] },
}

/** Secondary runtime check (used where docs are fetched and classified in Node.js). */
const TEST_NAME_RE    = /^(playwright|precommit-\d+)|^test\s|\btest lead\b|playwright\s+(test|mobile)/i
...
export function isTestDoc(doc: Record<string, unknown>): boolean {
  const name  = String((doc.name  as string) || ((doc.answers as Record<string, string>)?.name  ?? ''))
