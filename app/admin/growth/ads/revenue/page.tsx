...
interface SyncResult {
  ...
  imported?: number; skipped?: number; total?: number  // single-source format
}
...
export default function RevenueAttributionPage() {
  ...
  const [tab,         setTab]         = useState<"funnel" | "keywords" | "campaigns" | "products" | "states" | "leads" | "diagnostics">("funnel")
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      const [rRes, lRes] = await Promise.all([
        ...
        fetch("/api/admin/growth/ads/revenue-attribution?view=leads&limit=100"),
      ])
  ...
  const loadDiagnostics = useCallback(async () => {
    const res = await fetch("/api/admin/growth/ads/revenue-attribution?view=diagnostics")
  ...
  const sync = async (source?: string) => {
    ...
    const res = await fetch("/api/admin/growth/ads/revenue-attribution", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    })
    const d = await res.json()
    setSyncResult(d as SyncResult)
    setSyncing(false)
    loadReport()
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => sync()} disabled={syncing}
              className="flex items-center gap-1.5 text-xs font-medium bg-brand-600 text-white px-3 py-2 rounded-lg hover:bg-brand-700 transition-colors disabled:opacity-50">
            ...
            {syncResult && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex gap-3 text-xs text-green-700">
                ...
                <div className="flex-1">
                  <p className="font-semibold mb-1">Sync complete — {syncResult.totalNew ?? syncResult.imported ?? 0} new leads imported</p>
                  {syncResult.rfq && <p>RFQ Popup: +{syncResult.rfq.imported} new · {syncResult.rfq.skipped} already imported · {syncResult.rfq.total} total</p>}
                  {syncResult.brochure && <p>Brochure Downloads: +{syncResult.brochure.imported} new · {syncResult.brochure.skipped} already imported</p>}
                  {syncResult.contact && <p>Contact Form: +{syncResult.contact.imported} new · {syncResult.contact.skipped} already imported</p>}
                </div>
                <button onClick={() => setSyncResult(null)} className="text-green-400 hover:text-green-600">✕</button>
              </div>
            )}

            {leadsTotal === 0 && !syncing && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 text-xs text-amber-700">
                ...
                <div>
                  ...
                  <p>Click "Sync All Sources" to import from RFQ popup form, brochure downloads, and contact forms. Then update lead stages manually as deals progress.</p>
                </div>
              </div>
            )}
              ...
              <div className="p-4">
                ...
                {tab === "leads" && (
                  <div className="overflow-x-auto">
                    ...
                    {leads.length === 0 && (
                      <p className="text-center text-xs text-gray-400 py-8">No leads yet. Click "Sync All Sources" to import.</p>
                    )}
                    {leadsTotal > leads.length && (
                      <p className="text-center text-xs text-gray-400 py-2">Showing {leads.length} of {leadsTotal} leads</p>
                    )}
                  </div>
                )}
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          {[
                            ...
                            { label: "Pending in RFQ",     value: diagnostics.pendingInRFQ,        color: "text-amber-600" },
                            { label: "Pending Brochure",   value: diagnostics.pendingInBrochure,   color: "text-amber-600" },
                            { label: "Pending Contact",    value: diagnostics.pendingInSubmissions, color: "text-amber-600" },
                            { label: "With UTM Data",      value: diagnostics.withUTM,             color: "text-green-600" },
                        ...
                        <div className="bg-white rounded-xl border border-gray-200">
                          <div className="px-4 py-3 border-b border-gray-100">
                            <h4 className="text-xs font-semibold text-gray-700">By Source</h4>
                          </div>
                          <div className="p-4 grid grid-cols-4 gap-3">
                            {Object.entries(diagnostics.bySource).map(([src, count]) => (
                              <div key={src} className="bg-gray-50 rounded-lg p-3 text-center">
                                <p className="text-xl font-bold text-gray-800">{count as number}</p>
                                <p className="text-[10px] text-gray-400">{src.replace(/_/g, " ")}</p>
                              </div>
                            ))}
                          </div>
                        </div>

                        {diagnostics.sampleUnmatched.length > 0 && (
                          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                            <p className="text-xs font-semibold text-amber-800 mb-2">Sample leads without UTM attribution</p>
                            <div className="space-y-1.5">
                              {diagnostics.sampleUnmatched.map(l => (
                                <div key={l.leadId} className="flex items-center gap-2 text-[11px] text-amber-700">
                                  <span className="font-medium">{l.name ?? l.phone ?? l.leadId}</span>
                        ...
                        {(diagnostics.pendingInRFQ + diagnostics.pendingInBrochure + diagnostics.pendingInSubmissions) > 0 && (
                          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3 items-center">
                            ...
                            <div className="flex-1 text-xs text-blue-700">
                              <span className="font-semibold">{diagnostics.pendingInRFQ + diagnostics.pendingInBrochure + diagnostics.pendingInSubmissions} unimported leads</span> exist across your sources.
                            </div>
                            <button onClick={() => sync()} disabled={syncing}
