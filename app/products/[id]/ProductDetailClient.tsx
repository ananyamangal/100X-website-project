...
import { ChevronLeft, ChevronRight, Download, MessageCircle, Play, CheckCircle2, ArrowRight, Star, Zap } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        {current?.kind === 'youtube' ? (
          videoPlaying ? (
            ...
            >
              ...
              <div className="absolute w-20 h-20 bg-red-600 rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform">
                <Play size={32} className="text-white ml-1.5" fill="white" />
              </div>
              <p className="absolute bottom-4 left-0 right-0 text-center text-white text-sm font-500">Watch Product Demo</p>
            </button>
          )
        ) : (
          // eslint-disable-next-line @next/next/no-img-element
        ...
        {total > 1 && (
          <>
            <button onClick={prev} aria-label="Previous" className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-white/80 hover:bg-white shadow-md flex items-center justify-center transition-all hover:scale-105">
              <ChevronLeft size={20} />
            </button>
            <button onClick={next} aria-label="Next" className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-white/80 hover:bg-white shadow-md flex items-center justify-center transition-all hover:scale-105">
        ...
        {total > 1 && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
            {mediaItems.map((_, i) => (
              <button key={i} onClick={() => { setActiveIndex(i); setVideoPlaying(false); }} className={`w-1.5 h-1.5 rounded-full transition-all ${i === activeIndex ? 'w-4 bg-brand-600' : 'bg-white/60 hover:bg-white'}`} />
            ))}
          </div>
        )}
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {mediaItems.slice(0, 6).map((item, i) => (
            <button
              ...
              className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${i === activeIndex ? 'border-brand-600 shadow-md' : 'border-gray-200 hover:border-brand-300'}`}
            >
              {item.kind === 'youtube' ? (
                <>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
...
export default function ProductDetailClient({ productId }: { productId: string }) {
  ...
  // Parse performance metrics: "Value | Label | Description"
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="container mx-auto px-4 md:px-6">
            ...
            <ScrollReveal animation="fade-up" delay={100}>
              <div className="flex flex-wrap justify-center items-center gap-6 md:gap-10">
                {/* Text certifications from the new CMS field */}
      ...
      {specs.length > 0 && (
        <section className="py-20 md:py-24 bg-gray-50" id="specs">
          <div className="container mx-auto px-4 md:px-6">
            <ScrollReveal animation="fade-up" className="mb-12">
            ...
            <ScrollReveal animation="fade-up" delay={100}>
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                {specs.map((spec, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center justify-between px-6 py-4 gap-4 ${i !== specs.length - 1 ? 'border-b border-gray-50' : ''} hover:bg-gray-50/50 transition-colors`}>
                      <span className="text-gray-500 text-sm font-400 min-w-0 flex-1">{label.trim()}</span>
                      <span className="text-gray-900 font-600 text-sm text-right flex-shrink-0">{value || spec}</span>
                    </div>
                  );
                })}
              </div>
