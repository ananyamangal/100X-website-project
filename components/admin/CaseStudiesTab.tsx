...
export function CaseStudiesTab() {
  ...
  const publishAllDrafts = async () => {
    const drafts = studies.filter((s) => s.autoGenerated && !s.published)
    if (drafts.length === 0) {
      setMsg({ type: "success", text: "No auto-generated drafts to publish." })
      return
    }
    if (!confirm(`Publish ${drafts.length} auto-generated draft case ${drafts.length === 1 ? "study" : "studies"} to the live website?`)) return
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {studies.some((s) => s.autoGenerated && !s.published) && (
            <Button
              variant="outline"
            ...
            >
              {publishingAll ? "Publishing…" : `Publish All Auto-Generated Drafts (${studies.filter(s => s.autoGenerated && !s.published).length})`}
            </Button>
          )}
