...
export function CaseStudiesTab() {
  ...
  const publishAllDrafts = async () => {
    const drafts = studies.filter((s) => !s.published && !s.isSample)
    if (drafts.length === 0) {
      setMsg({ type: "success", text: "No unpublished drafts to publish." })
      return
    }
    if (!confirm(`Publish all ${drafts.length} draft case ${drafts.length === 1 ? "study" : "studies"} to the live website?`)) return
    setPublishingAll(true)
    try {
      const res = await fetch("/api/admin/case-studies/publish-all", { method: "POST" })
      const data = await res.json()
      if (data.ok) {
        setMsg({ type: "success", text: data.message || `Published ${data.published} case studies.` })
        load()
      } else {
        setMsg({ type: "error", text: "Publish failed." })
      }
    } catch {
      setMsg({ type: "error", text: "Network error." })
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {studies.some((s) => !s.published && !s.isSample) && (
            <Button
              variant="outline"
              onClick={publishAllDrafts}
              disabled={publishingAll}
              className="bg-transparent text-sm border-purple-200 text-purple-700 hover:bg-purple-50"
            >
              {publishingAll ? "Publishing…" : `Publish All Drafts (${studies.filter(s => !s.published && !s.isSample).length})`}
            </Button>
          )}
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <label className={`inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-300 rounded-lg text-sm cursor-pointer hover:bg-gray-50 transition-colors ${uploading ? "opacity-50" : ""}`}>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-brand-300 text-brand-700 rounded-lg text-sm hover:bg-brand-50 transition-colors"
                  >
                    <Images size={14} />
                ...
                <div className="flex gap-2 mb-3">
                  <Input
                    ...
                    placeholder="Paste image URL (Cloudinary, https://…)"
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {form.images.map((url: string, i: number) => (
                      <div key={i} className="relative group">
                        <img src={url} alt="" className="w-24 h-18 object-cover rounded-lg border border-gray-200" style={{ height: "4.5rem" }} />
                        <button type="button" onClick={() => set("images", form.images.filter((_: string, j: number) => j !== i))} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                        {i === 0 && <span className="absolute bottom-0 left-0 right-0 text-[9px] text-center bg-black/50 text-white py-0.5 rounded-b-lg">Primary</span>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              {showMediaPicker && (
                <MediaLibraryPicker
                  onSelect={url => set("images", [...(form.images || []), url])}
