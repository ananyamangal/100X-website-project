"use strict"
// One-shot: re-extract from cached text files and update the 10 pilot records in MongoDB.
// Does NOT re-download PDFs — uses audit/enrichment/text/ files already on disk.
// Run: node scripts/gem-repair-pilot.js
...
// ── Helpers (copied from gem-enrich-contracts.js) ─────────────────────────
const STATES_LONG = [
...
function detectState(t) {
  for (const s of STATES_LONG) { if (new RegExp(`\\b${s}\\b`, "i").test(t)) return s }
  return null
}
...
function extractPin(t) { const m = t.match(/\b([1-9][0-9]{5})\b/); return m ? m[1] : null }
...
;(async () => {
  ...
  const gcRaw = client.db().collection("gem_contracts_raw")

  console.log(`\nRepairing ${files.length} pilot records from cached text files...\n`)

  let updated = 0, failed = 0
  const now = new Date()

  for (const f of files) {
    const gemcSlug = f.replace(".txt", "")
    const gemc_no  = gemcSlug.replace(/_/g, "-").replace(/^GEMC-/, "GEMC-")
    const rawText  = fs.readFileSync(path.join(TEXT_DIR, f), "utf8")
    const { fields, confidence, found, total } = extractFields(rawText)
    ...
    try {
      await gc.updateOne(
        ...
        { $set: {
            ...
            seller_msme:           fields.seller_msme_number ? "yes" : null,
            ...
            buyer_designation:     fields.buyer_designation,
      ...
      console.log(`  ✓ ${gemc_no}  conf=${confidence}%  seller="${fields.seller_name}"  value=${fields.contract_value_pdf}`)
      updated++
    } catch (e) {
      console.error(`  ✗ ${gemc_no}  ${e.message}`)
      failed++
    }
  }
